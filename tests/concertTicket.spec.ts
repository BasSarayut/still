...
test('concert tickets preserve personal fields, crop, paper and exported artwork', async ({ page }, testInfo) => {
  ...
  await page.locator('#template').selectOption('concertTicket');
  ...
  await page.locator('#ticket-value').fill('คืนที่เราร้องเพลง\n思い出の夜');
  await page.locator('#ticket-field').selectOption('seat');
  await page.locator('#ticket-label').fill('STANDING'); await page.locator('#ticket-value').fill('007');
  await page.locator('#zoom').fill('2'); await page.locator('#background').fill('#c7ced4');
  ...
  await page.getByText('Ticket layout', { exact: true }).click();
  await page.getByLabel('Ticket rotation', { exact: true }).fill('8');
  ...
  await page.getByText('Stub & perforation', { exact: true }).click();
  await page.locator('#ticket-stub-side').selectOption('start'); await page.locator('#ticket-perforation').selectOption('dash');
  await page.getByText('Ticket colors & paper', { exact: true }).click();
  await page.locator('#ticket-paper').fill('#f4d4ba'); await page.locator('#ticket-stubPaper').fill('#abc1b2');
  await page.locator('#ticket-texture').selectOption('fiber');
  await page.getByLabel('Texture strength', { exact: true }).fill('45');
  ...
  await page.locator('#ticket-field').selectOption('seat'); await expect(page.locator('#ticket-value')).toHaveValue('007');
  await page.getByText('Ticket layout', { exact: true }).click();
  await expect(page.getByLabel('Ticket rotation', { exact: true })).toHaveValue('8');
  await page.getByText('Ticket colors & paper', { exact: true }).click(); await expect(page.locator('#ticket-paper')).toHaveValue('#f4d4ba');
  await page.locator('#language').selectOption('ja'); await expect(page.locator('#ticket-value')).toHaveValue('007');
  await page.locator('#language').selectOption('th'); await expect(page.locator('#ticket-label')).toHaveValue('STANDING');
  await page.locator('#language').selectOption('en');
...
test('text-only tickets export without an image and stub controls reflow both orientations', async ({ page }, testInfo) => {
  ...
  await page.locator('#language').selectOption('en'); await page.locator('#template').selectOption('concertTicket');
  await page.getByText('Ticket layout', { exact: true }).click();
  await page.getByRole('switch', { name: 'Show photo', exact: true }).uncheck();
  await expect(page.locator('.artwork-hit')).toHaveCount(0);
  await expect(page.getByRole('button', { name: 'Download PNG' })).toBeEnabled();
  await page.getByText('Stub & perforation', { exact: true }).click();
  await page.getByRole('switch', { name: 'Use concert details on stub', exact: true }).uncheck();
  await page.locator('#ticket-stub-text').fill('THE NIGHT WE SANG TOGETHER');
  for (const orientation of ['portrait', 'landscape']) {
    await page.locator('#ticket-orientation').selectOption(orientation);
    for (const side of ['start', 'end']) {
      await page.locator('#ticket-stub-side').selectOption(side);
      await page.locator('.wallpaper').screenshot({ path: testInfo.outputPath(`${orientation}-${side}.png`) });
    }
  }
  await page.getByRole('switch', { name: 'Show ticket stub', exact: true }).uncheck();
  ...
  await page.locator('#template').selectOption('custom'); await expect(page.getByRole('button', { name: 'Download PNG' })).toBeDisabled();
});
