...
export default function PolaroidEditor({ value, copy, onChange, panel = 'style' }: Props & { panel?: 'text' | 'style' }) {
  ...
  return <div className="player-editor polaroid-editor">
    {panel === 'style' && <details className="player-details" open><summary>{copy.polaroidFrame}</summary>
    ...
    {panel === 'text' && <details className="player-details" open><summary>{copy.playerTypography}</summary>
    ...
    {panel === 'style' && <AdvancedSettings copy={copy}><details className="player-details" open><summary>{copy.polaroidDecorations}</summary>
