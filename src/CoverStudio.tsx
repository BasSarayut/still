...
export default function CoverStudio({ value, language, background, foreground, onChange, onColors }: { value: AlbumCover; language: Language; background: string; foreground: string | null; onChange: (value: AlbumCover) => void; onColors: (background: string, foreground: string | null, cover: AlbumCover) => void }) {
  ...
  return <div className="cover-studio-controls">
    ...
    <details className="inspector-advanced"><summary>{language === 'th' ? 'ปรับละเอียด' : language === 'ja' ? '詳細設定' : 'Advanced settings'}</summary><div className="inspector-advanced-content">
    <details className="cover-details" open><summary>{copy.effects}</summary><div className="cover-grid">{range('grain', copy.grain, 0, 1, 0.05)}{range('fade', copy.fade, 0, 1, 0.05)}{range('vignette', copy.vignette, 0, 1, 0.05)}{range('lightLeak', copy.lightLeak, 0, 1, 0.05)}{range('overlay', copy.overlay, 0, 1, 0.05)}</div></details>
