...
export type PolaroidPreset = 'classic' | 'clean' | 'diary' | 'cinema' | 'noir';
export type PolaroidSettings = {
  ...
  tape: 'none' | 'top' | 'corners'; tapeColor: string;
  showStamp: boolean; stamp: string;
  wallPattern: 'none' | 'grid' | 'dots'; paletteStyle: 'strip' | 'dots';
};
...
export function createPolaroidSettings(preset: PolaroidPreset = 'classic'): PolaroidSettings {
  const base: PolaroidSettings = {
    ...
    rotation: 0, position: 25.5, shadow: 28, paper: '#fffdf9', ink: null,
    ...
    showProgress: true, showPauseGlyph: true, showTimes: true, tape: 'none', tapeColor: '#d8c5a0',
    showStamp: false, stamp: '', wallPattern: 'none', paletteStyle: 'strip',
  };
  ...
  if (preset === 'diary') return { ...base, photoWidth: 330, padding: 20, rotation: -5, radius: 2, photoRadius: 0, paper: '#f7efd9', titleFont: 'serif', titleItalic: true, artistFont: 'mono', titleSize: 24, showProgress: false, showPauseGlyph: false, tape: 'top', showStamp: true, wallPattern: 'grid', paletteStyle: 'dots' };
...
export function restorePolaroidSettings(value: unknown): PolaroidSettings {
  ...
  const ranges = { photoWidth: [260, 404], padding: [8, 36], captionHeight: [80, 180], radius: [0, 24], photoRadius: [0, 24], rotation: [-10, 10], position: [15, 50], shadow: [0, 60], titleSize: [14, 32], artistSize: [10, 22], titleWeight: [100, 900], artistWeight: [100, 900] } as const;
  ...
  const options = { photoFormat: ['square', 'portrait', 'landscape'], align: ['left', 'center', 'right'], tape: ['none', 'top', 'corners'], wallPattern: ['none', 'grid', 'dots'], paletteStyle: ['strip', 'dots'] } as const;
  ...
  for (const key of ['titleItalic', 'artistItalic', 'showProgress', 'showPauseGlyph', 'showTimes', 'showStamp'] as const) if (typeof saved[key] === 'boolean') result[key] = saved[key];
...
export function polaroidLayout(height: number, settings: PolaroidSettings, showPalette: boolean) {
  ...
  const reserved = showPalette ? 190 : 110;
