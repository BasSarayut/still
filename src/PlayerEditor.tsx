...
import { createPlayerSettings, type PlayerSettings, type PlayerPreset } from './musicPlayer';
...
export function PlayerPresets({ value, copy, onChange }: Props) {
  return <div className="player-presets"><p className="field-hint">{copy.playerPresetHint}</p><div>{(['classic', 'dark', 'minimal'] as PlayerPreset[]).map((preset, index) => {
    const start = createPlayerSettings(preset);
    const selected = (Object.keys(start) as (keyof PlayerSettings)[]).every(key => value[key] === start[key]);
    return <button type="button" key={preset} className={`player-preset ${preset}`} aria-pressed={selected} onClick={() => onChange(start)}><span className="preset-art" aria-hidden="true"><i /><b /><em /></span>{[copy.playerClassic, copy.playerDark, copy.playerMinimal][index]}</button>;
  })}</div></div>;
}

export function PlayerBackground({ value, copy, onChange }: Props) {
  return <div className="player-background"><label className="field-label" htmlFor="player-background-mode">{copy.playerBackground}</label><select id="player-background-mode" value={value.backgroundMode} onChange={event => onChange({ ...value, backgroundMode: event.target.value as PlayerSettings['backgroundMode'] })}><option value="solid">{copy.playerSolid}</option><option value="gradient">{copy.playerGradient}</option><option value="photo">{copy.playerPhoto}</option></select>
    ...
    {value.backgroundMode === 'photo' && <><p className="field-hint">{copy.playerPhotoHint}</p><Range label={copy.playerDarkness} value={value.darkness} min={0} max={80} suffix="%" onChange={darkness => onChange({ ...value, darkness })} /></>}
  </div>;
}
...
export default function PlayerEditor({ value, copy, onChange, ink }: Props & { ink: string }) {
  ...
  return <div className="player-editor">
    <details className="player-details"><summary>{copy.playerArtwork}</summary>
      ...
      {range('position', copy.playerPosition, 15, 42, 0.5, '%')}{range('gap', copy.playerGap, 8, 48)}
    ...
    <details className="player-details"><summary>{copy.playerPlayback}</summary>
      ...
      {value.showProgress && <>{toggle('showTimes', copy.playerShowTimes)}{value.showTimes && select('timeDisplay', copy.playerTimeDisplay, [['duration', copy.duration], ['remaining', copy.playerRemaining]])}</>}
